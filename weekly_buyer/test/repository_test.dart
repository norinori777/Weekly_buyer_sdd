...
void main() {
  test('creates a Monday-start weekly list and reuses item masters', () async {
    final database = AppDatabase(executor: NativeDatabase.memory());
    addTearDown(database.close);

    final repository = WeeklyShoppingRepository(database);
    await database.into(database.categories).insert(
          CategoriesCompanion.insert(name: '食品', sortOrder: const drift.Value(0)),
        );
    ...
    final snapshot = await repository.loadWeek(DateTime(2026, 4, 22));

    expect(snapshot.weekRange.start, DateTime(2026, 4, 20));
    expect(snapshot.weekRange.end, DateTime(2026, 4, 26));
    expect(snapshot.sections.firstWhere((section) => section.section == ShoppingSection.morning).items, hasLength(1));
    ...
    expect(itemMasters, hasLength(1));
    expect(itemMasters.single.name, '牛乳');
  });

  test('toggles purchase state and supports undo of the latest purchase', () async {
    final database = AppDatabase(executor: NativeDatabase.memory());
    addTearDown(database.close);
    ...
    expect(snapshot.sections.firstWhere((section) => section.section == ShoppingSection.other).items, hasLength(1));
  });
}