...
**画面遷移の考え方**
- 購入リスト画面を起点にして、商品追加画面へは同じ画面枠の中で切り替える
- 画面をまたいで戻るより、下部タブまたは上部セグメントで「購入リスト」「商品追加」を固定的に切り替える
- 週の選択状態は共通にして、どちらの画面でも同じ週を見続けられるようにする
- 商品追加は、一覧画面の右下ボタンからも開けるようにして、入力導線を短くする
- 追加完了後は、保存してそのまま商品追加画面に残るか、購入リスト画面へ戻るかを操作ごとに選べるようにする

**おすすめの画面構成案**
- `MainShell`
  - `BottomNavigationBar` で `購入リスト` / `商品追加` / `設定` を切り替える
  - `購入リスト` と `商品追加` は同じ週の状態を共有する
  - `設定` は独立画面ではなく、必要時だけ開く補助画面にする
- `購入リスト画面`
  - 週全体の一覧確認と購入済みチェックを担当する
  - 右下の `＋` で商品追加のボトムシートを直接開く
  - 左フリック、チェック、元に戻すを集約する
- `商品追加画面`
  - 朝昼夜やその他の入力をまとめて行う
  - 追加候補の選択と新規登録を担当する
  - 入力完了後に購入リストへ戻るショートカットを置く

この構成にすると、購入リストで見ながら追加し、追加画面でまとめて入力し、再び購入リストに戻る流れが1画面遷移以内で済みます。特に「画面を移動した感」を減らしたいなら、購入リスト画面を基準にして追加ボトムシートを中心導線にするのが一番扱いやすいです。
...
**おすすめの画面構成**
- 上部に「今週の曜日タブ」
  - 月〜日とその他を横スクロールか8分割で表示
  - 今日の曜日だけ強調
  - その日を切り替えると、商品がその日に紐づいて表示される
- 上部から下に「商品リスト」
  - その曜日に追加した商品を一覧表示
  - チェック済み、未購入、数量不足を分ける
- 画面下に固定の「追加ボタン」
